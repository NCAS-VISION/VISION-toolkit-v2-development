...
def irc_interp_ap(xap, latitude):
    ...
    lats_ap = np.arange(18.) * 10 - 85  # IDL: dindgen(18) * 10 - 85d0
    setup_linear(lats_ap, latitude, i0, i1, w0, w1, /vl)
    np = len(latitude) 
    nz = len(xap(*, 0)) 
...
def irc_ak_exp(ak, evecs, pf, sp, nz, nev):
    ...
    # IDL: matrix_multiply(evecs,ak_101, /btr), noting btr => .T.T (cancel e/o)
    ak_101 = np.dot(evecs.T, ak_101).T  # Map to rttov levels
...
def ims_rd_co4ak(fi, approx=approx):
    ...
    c_sc = np.zeros((nsc, nret), Float)  # IDL: dblarr(nsc, nret)  # CO sub-column average mixing ratios / ppmv 
    ak_sc = np.zeros((nsc, nz, nret), Float)  # IDL: dblarr(nsc, nz, nret)  # Averaging kernels for retrieved sub-column wrt true profile vmr / ppmv/ppmv 
    sx_sc = np.zeros((nsc, nsc, nret), Float)  # IDL: dblarr(nsc, nsc, nret)  # Total (Noise + smoothing) covariance for sub-col.avg.vmrs / ppmv^2 
    sn_sc = np.zeros((nsc, nsc, nret), Float)  # IDL: dblarr(nsc, nsc, nret)  # Noise covariance  / ppmv^2 
    c_apc_sc = np.zeros((nsc, nret), Float)  # IDL: dblarr(nsc, nret)  # A priori contribution to each sub-column / ppmv 
    c_err_sc = np.zeros((nsc, nret), Float)  # IDL: dblarr(nsc, nret)  # Estimated total standard deviation of retrieved sub-cols / ppmv 
    c_noise_sc = np.zeros((nsc, nret), Float)  # IDL: dblarr(nsc, nret)  # Estimated noise standard deviation / ppmv
    ...
    for iret in np.arange(0l, nret): 
        ...
        c_apc_sc(0,iret) = c_ap_sc - matrix_multiply(
            ak_sc(*, *, iret), c_ap_vmr)
        ...
        sx_sc(0, 0, iret) = matrix_multiply(
            matrix_multiply(sx_vmr, msc), msc, /atr) 
        sn_sc(0, 0, iret) = matrix_multiply(
            matrix_multiply(sx_vmr, msc), msc, /atr)
